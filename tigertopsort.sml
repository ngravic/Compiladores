structure tigertopsort:> tigertopsort = struct
    ...
    let fun buscaRecs [] res = res
          | buscaRecs ({name, ty = RecordTy lf}::t) res =
                    let fun genrecs [] _ = []
                          ...
                          | genrecs _ _ = error ErrorInterno "topsort42" ~1
                    in buscaRecs t ((name, TRecord(genrecs lf 0, ref ()))::res) end
          | buscaRecs ({name, ty = ArrayTy ty}::t) res =
                        buscaRecs t ((name, TArray(ref (TTipo ty), ref ()))::res)
          | buscaRecs (_::t) res = buscaRecs t res
    in buscaRecs lt [] end

fun genPares lt =
    let val lrecs = buscaArrRecords lt
        fun genP [] res = res
  ...
  | procesa (sorted as (h::t)) (pares:{name:symbol, ty:ty} list) env recs =
        let fun filt h {name, ty = NameTy t} = h = t
            ...
            val ttopt = case tabBusca h env of
                        SOME t => SOME t
                      | _ => case List.find ((h ls op=) o #1) recs of
                             SOME (n, tr) => (tabRInserta h tr env;
                                              SOME tr) (* OJOOOOOOOOOOOOOOOO *)
                             | _ => error (NoExiste h) "topsort70" ~1
            fun aux tt ({name, ty = NameTy ty}, env') = tabRInserta name tt env'
              | aux tt ({name, ty = ArrayTy s}, env') =
                               let val (k, v) = getOptn (List.find ((name ls op=) o #1) recs)
                                                         ErrorInterno "topsort74" ~1
                               in tabRInserta k v env' end
              | aux tt ({name, ty = RecordTy s}, env') =
                               let val (k, v) =
                                   getOptn (List.find ((name ls op=) o #1) recs)
                                   ErrorInterno "topsort79" ~1
                               in tabRInserta k v env' end
            val env' = case ttopt of SOME tt => List.foldr (aux tt) env ps
                                   | _ => env
...
fun fijaNONE [] env = env
    | fijaNONE ((name, TArray (ar as (ref (TTipo s)), u))::t) env =
          (case tabBusca s env of
           NONE => error ErrorInterno "topsort88" ~1
